...
import { ArtifactPanel, Artifact } from "../Artifacts";
...
export const Chat: FC<Props> = ({ messages, loading, streaming, onSend, onReset }) => {
  ...
  return (
    <div className="flex h-full min-h-0 flex-1">
      ...
      <div className={`flex min-h-0 flex-1 flex-col transition-all duration-300 ${
        ...
        {showScrollButton && (
          <button
            ...
            className={`fixed bottom-24 rounded-full bg-pink-500 p-3 text-white shadow-lg transition-all duration-200 hover:scale-110 hover:bg-pink-600 focus:outline-none focus:ring-2 focus:ring-pink-500 focus:ring-offset-2 ${
              selectedArtifact ? 'right-[calc(50%+1.5rem)]' : 'right-6'
            }`}
      ...
      {/* Artifact Panel - Side panel for viewing reports/code */}
      {selectedArtifact && (
        <div className="w-1/2 min-h-0 flex-shrink-0 animate-slideInRight">
          <ArtifactPanel
